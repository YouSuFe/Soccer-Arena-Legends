...
public class MatchplayMatchmaker : IDisposable
{
            ...
            try
            {
                while (!cancelToken.IsCancellationRequested)
                {
                    ...
                    if (checkTicket.Type == typeof(MultiplayAssignment))
                    {
                        ...
                        Debug.Log($"Polled Ticket: {lastUsedTicket} Status: {matchAssignment.Status} User Data: {data.userGamePreferences.gameMode} {data.userGamePreferences.map} ");
                    }
    ...
    public async Task<MatchmakingResult> MatchmakeForTeam(List<UserData> teamData, Action<MatchmakerPollingResult> onMatchmakeResponse)
    {
        cancelToken = new CancellationTokenSource();

        // Convert team data to player objects
        List<Player> players = new List<Player>();
        foreach (var user in teamData)
        {
            players.Add(new Player(user.userAuthId, user.userGamePreferences));
        }

        string queueName = string.Empty;
        if (teamData != null && teamData.Count > 0)
        {
            queueName = teamData[0].userGamePreferences.ToMultiplayQueue();
        }
        else
        {
            Debug.LogWarning($"Team Data is empty! Due to that Matchmaker Queue name is also empty");
        }

        try
        {
            IsMatchmaking = true;
            CreateTicketResponse createResult = await MatchmakerService.Instance.CreateTicketAsync(players, new CreateTicketOptions(queueName));
                ...
                if (checkTicket.Type == typeof(MultiplayAssignment))
                {
                    ...
                    if (matchAssignment.Status == MultiplayAssignment.StatusOptions.Found)
                    {
                        return ReturnMatchResult(MatchmakerPollingResult.Success, "", matchAssignment);
                             ...
                             matchAssignment.Status == MultiplayAssignment.StatusOptions.Failed)
                    {
                        return ReturnMatchResult(MatchmakerPollingResult.MatchAssignmentError, $"Ticket: {lastUsedTicket} - {matchAssignment.Status}", null);
                    }
                    Debug.Log($"Polled Ticket: {lastUsedTicket} Status: {matchAssignment.Status} User Data: {teamData[0].userGamePreferences.gameMode} {teamData[0].userGamePreferences.map} ");
